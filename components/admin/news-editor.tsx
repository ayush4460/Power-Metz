...
export function NewsEditorForm({ initialData = null }: { initialData?: any }) {
  ...
  const handleUploadCover = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    uploadData.append('folder', `powermetz/news/${formData.slug || 'drafts'}/cover`)
    try {
      ...
      <PageHeader 
        title={
          <div className="flex items-center gap-2">
            <Link href="/admin/newss" className="p-1 hover:bg-muted rounded-full transition-colors text-muted-foreground mr-1">
              <ArrowLeft className="w-5 h-5" />
            </Link>
            {initialData ? 'Edit News Article' : 'Create New News Article'}
          </div>
        }
      />
      ...
      <form onSubmit={handleSubmit}>
        <div className="bg-white p-6 sm:p-8 rounded-2xl border border-border shadow-sm space-y-6">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground mb-2">Shown in the news listing page below the title.</p>
            <textarea
          ...
          <div className="flex items-center justify-end gap-3 pt-4 border-t border-border">
            <Button type="button" variant="outline" onClick={() => router.push('/admin/newss')} disabled={isSaving}>
              Cancel
            </Button>
            <Button type="submit" className="bg-primary hover:bg-primary/90 text-white" disabled={isSaving}>
              {isSaving ? <Loader2 className="w-4 h-4 mr-2 animate-spin" /> : <Save className="w-4 h-4 mr-2" />}
              {isSaving ? 'Saving...' : initialData ? 'Save Changes' : 'Publish News'}
            </Button>
          </div>
