...
export default async function VendorSetupPage({ searchParams }: { searchParams: Promise<{ token?: string }> }) {
  ...
  if (!token) {
    leftContent = (
      <div className="w-full max-w-sm mx-auto text-center">
        ...
        <H3 className="mb-2">Invalid Setup Link</H3>
        <Paragraph className="text-slate-500 text-sm">
          This setup link is invalid or missing the security token. Please check the link in your email and try again.
        </Paragraph>
      </div>
    );
  } else {
    ...
    const isExpired = user?.setupTokenExpiresAt ? user.setupTokenExpiresAt < new Date() : false
    ...
    if (!user || isExpired) {
      leftContent = (
        <div className="w-full max-w-sm mx-auto text-center">
          ...
          <H3 className="mb-2">Link Expired or Used</H3>
          <Paragraph className="text-slate-500 text-sm mb-4">
            For security reasons, this setup link has either expired after 24 hours or has already been used to set up an account.
          </Paragraph>
          <p className="text-xs text-slate-400">
            If you need a new setup link, please contact the administration team.
          </p>
        </div>
      );
    } else {
      leftContent = (
        <div className="w-full max-w-sm mx-auto">
          <div className="mb-10 text-center lg:text-left">
            <H3 className="text-3xl font-extrabold text-slate-800 tracking-tight">Secure Account Setup</H3>
            <Paragraph className="text-slate-500 mt-2 text-sm font-medium">
              Create a password for <strong>{user.email}</strong> to activate your vendor account.
            </Paragraph>
          </div>
