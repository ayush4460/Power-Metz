...
import { setupVendorPassword } from "./actions"
...
export function SetupForm({ token }: { token: string }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsPending(true)
    const result = await setupVendorPassword(token, password)
    setIsPending(false)
    ...
    return (
      <div className="text-center space-y-4">
        ...
        <p className="text-muted-foreground">
          Your secure vendor account has been successfully set up.
        </p>
        <div className="pt-4">
          <a href="/vendor/login" className="inline-block bg-[#F58220] hover:bg-[#F58220]/90 text-white font-medium py-2 px-6 rounded-lg transition-colors">
            Proceed to Login
          </a>
        </div>
      </div>
