...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const currentPost = await prisma.news.findUnique({ where: { id } })
...
export async function DELETE(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (post.isDeleted) {
      ...
      await prisma.auditLog.create({
        data: { action: 'DELETE_POST_PERMANENT', entityId: id }
      })
    } else {
      await prisma.news.update({ where: { id }, data: { isDeleted: true } })
      await prisma.auditLog.create({
        data: { action: 'DELETE_POST_SOFT', entityId: id }
      })
    }
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (data.action === 'restore') {
      await prisma.news.update({ where: { id }, data: { isDeleted: false } })
      await prisma.auditLog.create({ data: { action: 'RESTORE_POST', entityId: id } })
      return NextResponse.json({ success: true })
    }
